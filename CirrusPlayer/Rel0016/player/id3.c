...
{
    unsigned long ulFlags, ulValid, ulTagLength, ulTemp, ulIdx;
                ...
                if(!(pucBuffer[ulStart + 9] & 0xc0))
                {
                    ...
                    if(pcTag[0] == 'T')
                    {
                        ...
                        if(pucBuffer[ulStart] == 0)
                        {
                            ...
                            // converting each to Unicode as it is copied into
                            // the output buffer.
                            //
                            for(ulIdx = 0;
                                (ulIdx < (ulValueLength - 1)) &&
                                (ulIdx < ulTemp);
                                ulIdx++)
                            {
                                pusValue[ulIdx] =
                                    pucBuffer[ulStart + ulIdx + 1];
                            }
                        ...
                        //
                        // Extract the string if the text encoding is Unicode.
                        //
                        if(pucBuffer[ulStart] == 1)
                        {
                            //
                            // Copy the string from the tag frame.
                            //
                            memcpy(pusValue, pucBuffer + ulStart + 3,
                                   ulValueLength * 2);

                            //
                            // See if the string is encoded in little-endian
                            // or big-endian format.
                            //
                            if((pucBuffer[ulStart + 1] == 0xfe) &&
                               (pucBuffer[ulStart + 2] == 0xff))
                            {
                                //
                                // The string is encoded in big-endian format,
                                // so we must byte swap each character in the
                                // string.
                                //
                                for(ulIdx = 0; ulIdx < ulValueLength; ulIdx++)
                                {
                                    pusValue[ulIdx] = (pusValue[ulIdx] >> 8) |
                                                      (pusValue[ulIdx] << 8);
                                }
                            }

                            //
                            // Null terminate that string at the appropriate
                            // place, either at the end of the string, or at
                            // the end of the buffer used to hold the string,
                            // whichever is less.
                            //
                            if((ulTemp / 2) < ulValueLength)
                            {
                                pusValue[(ulTemp / 2) - 1] = 0;
                            }
                            else
                            {
                                pusValue[ulValueLength - 1] = 0;
                            }
    ...
    if(ID3IsV1Tag(pucBuffer + ((ulLength - 128) & 511)))
    {
        ...
        if(memcmp(pcTag, "TPE1", 4) == 0)
        {
            ...
            //
            ulTemp = ulValueLength < 30 ? ulValueLength : 30;
            for(ulIdx = 0; ulIdx < ulTemp; ulIdx++)
            {
                pusValue[ulIdx] =
                    pucBuffer[((ulLength - 128) & 511) + ulIdx + 33];
            }
        }
        ...
        else if(memcmp(pcTag, "TALB", 4) == 0)
        {
            ...
            {
                pusValue[ulIdx] =
                    pucBuffer[((ulLength - 128) & 511) + ulIdx + 63];
            }
        }
        ...
        else if(memcmp(pcTag, "TIT2", 4) == 0)
        {
            ...
            {
                pusValue[ulIdx] =
                    pucBuffer[((ulLength - 128) & 511) + ulIdx + 3];
            }
        }
        ...
        else if(memcmp(pcTag, "TYER", 4) == 0)
        {
            ...
            //
            ulTemp = ulValueLength < 4 ? ulValueLength : 4;
            for(ulIdx = 0; ulIdx < ulTemp; ulIdx++)
            {
                pusValue[ulIdx] =
                    pucBuffer[((ulLength - 128) & 511) + ulIdx + 93];
            }
        }
        ...
        //
        // Null terminate the string and remove any trailing spaces that may be
        // in the string.
        //
        while(pusValue[ulTemp - 1] == ' ')
        {
            pusValue[--ulTemp] = '\0';
        }
