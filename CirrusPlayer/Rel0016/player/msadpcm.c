             ...
             unsigned long ulParam4)
{
    ...
    switch(ulIoctl)
    {
        ...
        case IOCTL_CODEC_QUERY:
        {
            ...
            unsigned long ulValid, ulOffset, ulLength;
            ADPCMWaveFormat sWaveFormat;
            ...
            //
            ulValid = FSRead(pFile, pucScratch, 512);

            //
            // Make sure that this is a RIFF WAVE file.
            //
            if((ulValid < 12) || (pucScratch[0] != 'R') ||
               (pucScratch[1] != 'I') || (pucScratch[2] != 'F') ||
               (pucScratch[3] != 'F') || (pucScratch[8] != 'W') ||
               (pucScratch[9] != 'A') || (pucScratch[10] != 'V') ||
               (pucScratch[11] != 'E'))
            {
                return(0);
            }
            ...
            // Find the "fmt " chunk.
            //
            ulOffset = CodecFindRIFFChunk(pucScratch, 512, 12, "fmt ");
            if(ulOffset == -1)
            {
                return(0);
            }
            ...
            //
            if((ulOffset + 8) > ulValid)
            {
                //
                // We could not find the "fmt " chunk in the first 512 bytes of
                // the file, so we can not decode this file.
                //
                return(0);
            }
            ...
            // Get the length of the "fmt " chunk.
            //
            ulLength = ((pucScratch[ulOffset + 7] << 24) |
                        (pucScratch[ulOffset + 6] << 16) |
                        (pucScratch[ulOffset + 5] << 8) |
                        pucScratch[ulOffset + 4]);

            //
            // If the length of the "fmt " chunk is not the same as the size of
            // the MS ADPCM wave format sturcture, then we can not decode this
            // file.
            //
            if(ulLength != sizeof(ADPCMWaveFormat))
            {
            ...
            // Make sure that the entire "fmt " chunk is in the data buffer.
            //
            if((ulOffset + ulLength + 8) > ulValid)
            {
                //
                // We could not find the entire "fmt " chunk in the first 512
                // bytes of the file, so we can not decode this file.
                //
                return(0);
            }
            ...
            //
            memcpy((void *)&sWaveFormat, pucScratch + ulOffset + 8,
                   sizeof(ADPCMWaveFormat));
        ...
        case IOCTL_CODEC_OPEN:
        {
            ...
            //
            if(((unsigned long)pMSADPCM + sizeof(tMSADPCM)) > ulExtentOfRAM)
            {
                return(0);
            }
        ...
        case IOCTL_CODEC_CAN_DECODE:
        {
            ...
            //
            if(BufferSpaceAvailable(pMSADPCM->pBuffer) <
               pMSADPCM->usSamplesPerBlock)
            {
                //
        ...
        //
        // Determine if the next frame of data can be encoded.
        //
        case IOCTL_CODEC_CAN_ENCODE:
        {
            ...
            //
            // See if there is enough data in the input buffer.
            //
            if(BufferDataAvailable(pMSADPCM->pBuffer) <
               pMSADPCM->usSamplesPerBlock)
            {
                //
                // There is not enough data available, so we can not encode the
                // next frame.
                //
                return(0);
            }
