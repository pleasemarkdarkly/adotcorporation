...
#define FLAG_TRUBASS_MASK                       0x000001e0
#define FLAG_TREBLE_BOOST_MASK                  0x00001e00
#define FLAG_SRS_MASK                           0x00001e00
...
#define FLAG_MENU_ITEM_MASK                     0x00780000
#define FLAG_BACKLIGHT                          0x00800000
...
#define FLAG_TRUBASS_SHIFT                      5
#define FLAG_TREBLE_BOOST_SHIFT                 9
#define FLAG_SRS_SHIFT                          9
...
static void UpdateRepeat(unsigned long ulIndex);
static void UpdateTreble(unsigned long ulIndex);
static void UpdateBass(unsigned long ulIndex);
#if defined(SUPPORT_SRS) || defined(SUPPORT_QSOUND)
static void UpdateWidth(unsigned long ulIndex);
#endif
#ifdef SUPPORT_SRS
static void UpdateTruBass(unsigned long ulIndex);
#endif
#ifdef SUPPORT_QSOUND
static void UpdateQSoundMode(unsigned long ulIndex);
#endif
static void UpdateBacklight(unsigned long ulIndex);
...
static const struct
{
    ...
    // menu item value.
    //
    void (*pfnUpdate)(unsigned long ulIndex);
...
} sMenuItems[] =
{
    ...
    // The "WOW" menu item.
    //
    { DisplayValueSlider, UpdateWidth, 0, 11, &sUI.ulSRSState, 0x00ff, 0 },
#endif
    ...
    //
    { DisplayValueSlider, UpdateTruBass, 0, 11, &sUI.ulSRSState, 0xff00, 8 },
#endif
    ...
    // The "Q2X" menu item.
    //
    { DisplayValueSlider, UpdateWidth, 0, 11, &sUI.ulQSoundState, 0x00ff, 0 },

    //
    // The "Q2X Mode" menu item.
    //
    { DisplayStringRing, UpdateQSoundMode, "Mode 0\0Mode 1", 2,
      &sUI.ulQSoundState, 0xff00, 8 },
#endif

    //
    // The "Treble" menu item.
    //
    { DisplayDecibleSlider, UpdateTreble, 0, 13, &sUI.ulFlags,
      FLAG_TREBLE_BOOST_MASK, FLAG_TREBLE_BOOST_SHIFT },

    //
    // The "Bass" menu item.
    //
    { DisplayDecibleSlider, UpdateBass, 0, 13, &sUI.ulFlags,
    ...
    //
    { DisplayStringRing, 0, "HW 1.00 SW 1.12", 1, 0, 0 },
...
DisplayStringRing(unsigned long ulIndex)
{
    ...
    //
    LCDPrintString(0, 0, 1, 17, pcValue, 15);
}
...
DisplayDecibleSlider(unsigned long ulIndex)
{
    ...
    //
    LCDPrintString(0, 5, 11, 17, cMsg, 14);
...
DisplayValueSlider(unsigned long ulIndex)
{
    ...
    // Print the value of the slider.
    //
    LCDPrintString(0, 5, 13, 17, cMsg, 15);
}
#endif
...
DisplayClock(unsigned long ulIndex)
{
    ...
    //
    LCDPrintString(0, 0, 3, 17, cMsg, 9);
    ...
    //
    LCDPrintString(0, 6, 9, 17, cMsg, 13);
}
...
DisplayCodecs(unsigned long ulIndex)
{
    ...
    for(i = 0, j = 0; i < 8; i++)
    {
        ...
        //
        if(CodecGetNameByIndex(i, &pcName) == 0)
        {
            break;
        }
    ...
    //
    LCDPrintString(0, 0, 1, 17, cMsg, 15);
}
...
DisplayGeekInfo(unsigned long ulIndex)
{
    ...
    //
    LCDPrintString(0, 0, 1, 17, cMsg, 4);
    ...
    //
    LCDPrintString(0, 4, 4, 17, cMsg, 6);
    ...
    //
    LCDPrintString((unsigned char *)(ucKHZ + 1), 6, 7, 17, cMsg, 10);
...
//****************************************************************************
static void
UpdateRepeat(unsigned long ulIndex)
{
    //
...
//****************************************************************************
//
// Updates the treble control in the output path based on the new treble
// value.
//
//****************************************************************************
static void
UpdateTreble(unsigned long ulIndex)
{
    unsigned long ulTemp;

    //
    // Get the new treble value.
    //
    ulTemp = (*(sMenuItems[ulIndex].pulFlags) &
              sMenuItems[ulIndex].ulFlagMask) >>
             sMenuItems[ulIndex].ulFlagShift;

    //
    // Provide the new treble value to the output data path.
    //
    OutputSetTreble(ulTemp);
}

//****************************************************************************
//
// Updates the bass control in the output path based on the new bass value.
//
//****************************************************************************
static void
UpdateBass(unsigned long ulIndex)
{
    unsigned long ulTemp;

    //
    // Get the new bass value.
    //
    ulTemp = (*(sMenuItems[ulIndex].pulFlags) &
              sMenuItems[ulIndex].ulFlagMask) >>
             sMenuItems[ulIndex].ulFlagShift;
    ...
    //
    // Provide the new bass value to the output data path.
    //
    OutputSetBass(ulTemp);
}

//****************************************************************************
//
// Updates the stereo enhancement control in the output path based on the new
// value.
//
//****************************************************************************
#if defined(SUPPORT_SRS) || defined(SUPPORT_QSOUND)
static void
UpdateWidth(unsigned long ulIndex)
{
    unsigned long ulTemp;
    ...
    //
    // Provide the new value to the output data path.
    //
    OutputSetWidth(ulTemp);
}
#endif

//****************************************************************************
//
// Updates the TruBass control in the output path based on the new value.
//
//****************************************************************************
#ifdef SUPPORT_SRS
static void
UpdateTruBass(unsigned long ulIndex)
{
    unsigned long ulTemp;
    ...
    // Provide the new value to the output data path.
    //
    OutputSetTruBass(ulTemp);
}
#endif

//****************************************************************************
//
// Updates the mode of the QSound control in the output path based on the new
// value.
//
//****************************************************************************
#ifdef SUPPORT_QSOUND
static void
UpdateQSoundMode(unsigned long ulIndex)
{
    //
    // Provide the new value to the output data path.
    //
    OutputSetQSoundMode(sUI.ulQSoundState >> 8);
}
#endif
...
//****************************************************************************
static void
UpdateBacklight(unsigned long ulIndex)
{
    //
...
UIFileLoaded(char *pcFileName, unsigned long ulTrack)
{
    ...
    //
    memcpy(sUI.pcFileName, pcFileName, 63);
...
UpdateMainDisplay(unsigned long ulUpdateFlags)
{
    ...
    if(sUI.ucTrack == 0)
    {
        ...
        //
        LCDDisplayImage(ucMaverickLogo, 0, 0, 16, 32);
        LCDPrintString(0, 0, 0, 0, "No", 2);
        LCDPrintString(0, 4, 0, 10, "Music", 4);
        LCDClear(0, 32, 16, 1);
    }
    else
    {
        ...
        if(ulUpdateFlags & FLAG_UPDATE_TRACK)
        {
            ...
            //
            LCDPrintString(ucTrack[2], 5, 4, 0, cStr, 6);
            ...
            if(AudibleIsAudibleProgram())
            {
                ...
                //
                pcSongName = (char *)AudibleGetTitle();
            }
            else
            {
#endif
            ...
            //
            if(pcSongName && *pcSongName && pcArtist && *pcArtist)
            {
                //
                // Display the artist and song name.
                //
                LCDPrintString(0, 0, 0, 11, pcArtist, 16);
                LCDPrintString(0, 0, 0, 22, pcSongName, 16);
            }

            //
            // Otherwise, see if there is a song name.
            //
            else if(pcSongName && *pcSongName)
            {
                //
                // Display the song name.
                //
                LCDClear(0, 11, 16, 4);
                LCDPrintString(0, 0, 0, 15, pcSongName, 16);
                LCDClear(0, 26, 16, 7);
            }
            ...
            //
            else if(pcArtist && *pcArtist)
            {
                //
                // Display the artist name and the name of the file.
                //
                LCDPrintString(0, 0, 0, 11, pcArtist, 16);
                LCDPrintString(0, 0, 0, 22, sUI.pcFileName, 16);
            }
            ...
            else
            {
                ...
                //
                LCDClear(0, 11, 16, 4);
                LCDPrintString(0, 0, 0, 15, sUI.pcFileName, 16);
                LCDClear(0, 26, 16, 7);
            }
        }
        ...
        if(ulUpdateFlags & FLAG_UPDATE_TIME)
        {
            ...
            else
            {
                ...
                //
                // Add the start of the Audible program.
                //
#ifdef SUPPORT_AUDIBLE
                ulTemp1 += AudibleGetProgramStart();
#endif
            ...
            //
            LCDPrintString(ucTime[2], 7, 8, 0, cStr, 13);
        }
        ...
        if(ulUpdateFlags & FLAG_UPDATE_VOLUME)
        {
            ...
            //
            LCDPrintString(ucVolume[1], 5, 14, 0, cStr, 16);
        }
    }
...
UpdateMenuDisplay(unsigned long ulUpdateFlags)
{
    ...
    //
    // See if the menu item value was updated and there is a routine for
    // effecting the change in other parts of the player.
    //
    if((ulUpdateFlags & FLAG_UPDATE_VALUE) &&
       (sMenuItems[ulMenuItem].pfnUpdate))
    {
        //
        // Reflect the new menu item value in the rest of the player.
        //
        (sMenuItems[ulMenuItem].pfnUpdate)(ulMenuItem);
    }
}
