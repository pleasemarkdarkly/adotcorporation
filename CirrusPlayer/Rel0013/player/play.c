...
OpenTrack(tFile *pFile)
{
    ...
    AudibleLoadMetaData(ulDrive, cFileName);
#endif
    ...
    UISetCurrentTime(ulDrive + AudibleGetProgramStart());
#endif
}
    ...
    while(1)
    {
        ...
        if(ulMode == MODE_PLAY)
        {
            ...
            if(CodecDecode() == 0)
            {
                ...
                if((ulSystemFlags & SYSFLAG_REPEAT_MASK) == SYSFLAG_REPEAT_ONE)
                {
                    ...
                    //
#ifdef SUPPORT_AUDIBLE
                    UISetCurrentTime(AudibleGetProgramStart());
#else
                    UISetCurrentTime(0);
#endif
                }
            ...
            else
            {
                ...
                //
                // Add the start time of the Audible program.
                //
#ifdef SUPPORT_AUDIBLE
                ulTime += AudibleGetProgramStart();
#endif
        ...
        else if(ulButtons & BUTTON_PREV)
        {
            ...
            if((ulTime > 3000) && (ulMode != MODE_STOP))
            {
                ...
                //
#ifdef SUPPORT_AUDIBLE
                UISetCurrentTime(ulTime + AudibleGetProgramStart());
#else
                UISetCurrentTime(ulTime);
#endif
            }
