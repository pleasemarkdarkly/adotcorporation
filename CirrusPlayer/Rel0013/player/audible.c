...
static struct
{
    ...
    //
    // The offset of the beginning of the program segment contained in the
...
//****************************************************************************
void
AudibleLoadMetaData(unsigned long ulDrive, char *pcFileName)
{
    tFile sFile;
    ...
    //
    if(FSOpen(&sFile, ulDrive, "\\audible.dat", FS_OPEN_READ) == 0)
    {
        return;
    }
    ...
    while(ulOffset)
    {
        ...
        for(ulIdx = 0; ulIdx < 4; ulIdx++)
        {
            ...
            // searching.
            //
            if(strncmp((char *)(pucBuffer + 4 + (ulIdx * 127)), pcFileName,
                       123) == 0)
            {
                break;
            }
    ...
    //
    memcpy(sAudible.cTitle, psMetaData->cTitle, 256);

    //
    // Save the starting time of this program.
    //
    sAudible.ulProgramStart = psMetaData->ulProgramStart;
...
AudibleUpdateMetaData(unsigned char *pucBuffer, unsigned long ulNewPosition)
{
    ...
    // Open the Audible meta-data file.
    //
    if(FSOpen(&sFile, sAudible.ulMetaDataOffset & 255, "\\audible.dat",
...
//****************************************************************************
//
// AudibleGetProgramStart returns the time position of the start of the
// current Audible program, expressed in milliseconds.
//
//****************************************************************************
unsigned long
AudibleGetProgramStart(void)
{
    //
    // Return the starting position of the current program if the current file
    // contains an Audible program.
    //
    if(sAudible.ulMetaDataOffset)
    {
        return(sAudible.ulProgramStart);
    }
...
AudibleGetPreviousSection(unsigned long ulTime)
{
    ...
    //
    pulSections = ((tAudibleMetaData *)(pucBuffer + 4))->ulSectionStart;
