...
typedef struct
{
    ...
    //
    unsigned char pucTitle[128];

    //
    // A buffer to contain the song author.
    //
    unsigned char pucAuthor[128];
         ...
         unsigned long ulParam3, unsigned long ulParam4)
{
    ...
    switch(ulIoctl)
    {
        ...
        case IOCTL_CODEC_OPEN:
        {
            ...
            //
            if(((ulParam1 + sizeof(tWMA)) & 0xFFFF) > 0x9600)
            {
                return(0);
            }
            ...
            else
            {
                ...
                //
                // Convert the strings from Unicode to ASCII (making the
                // assumption that the string is using only ASCII characters).
                //
                for(i = 2; i < 128; i += 2)
                {
                    pWMA->pucTitle[i / 2] = pWMA->pucTitle[i];
                    pWMA->pucAuthor[i / 2] = pWMA->pucAuthor[i];
                }
            }
