...
//****************************************************************************
//
// AACFillInputBuffer reads more data into the input bitstream buffer if there
// is space available.
//
//****************************************************************************
static void
AACFillInputBuffer(tAAC *pAAC)
{
    ...
    {
        //
        // Do we need to read up to the end of the input bitstream buffer?
        //
        ...
        else
        {
            ...
            //
            ulLength = ((long)pAAC->sBS.DataOut - (long)pAAC->sBS.DataIn) &
                       ~0x1ff;

            //
            // Read data into the data buffer.
            //
            FSReadBS(pAAC->pFile, pAAC->sBS.DataIn, ulLength);

            //
            // Move the write pointer in the bitstream structure.
            //
            pAAC->sBS.DataIn += ulLength / 4;
        }
    }
}
         ...
         unsigned long ulParam3, unsigned long ulParam4)
{
    ...
    switch(ulIoctl)
    {
        ...
        case IOCTL_CODEC_GETNAME:
        {
            ...
            // is the long name.
            //
            *ppcName = "AAC 2.0.0.0 Channel Low Complexity MPEG-2 AAC Decoder";
        ...
        case IOCTL_CODEC_OPEN:
        {
            ...
            //
            if(((ulParam1 + sizeof(tAAC) + AAC_DecoderSize) & 0xFFFF) > 0x9600)
            {
                return(0);
            }
        ...
        case IOCTL_CODEC_DECODE:
        {
            ...
            //
            // Read more data from the file if there is space.
            //
            AACFillInputBuffer(pAAC);
