...
GetValueString(ULONG ulValue)
{
    ...
    for(ulLoop = 1000000000; ulLoop; ulLoop /= 10)
    {
        ...
        if((ulValue >= ulLoop) || (ulLoop == 1))
        {
            ...
            // buffer.
            //
            cBuf[ulIdx++] = '0' + (char)((ulValue / ulLoop) % 10);
...
LoadFileList(void)
{
    ...
    //
    if(Maverick_OpenDir(g_ulDriveNum, "\\") == 0)
    {
        return;
    }
    ...
    //
    lIdx = 0;
    while(Maverick_ReadDir(pcBuffer, 1) != 0)
    {
        //
        ...
        // Open the file.
        //
        Maverick_Open(g_ulDriveNum, pcBuffer, 1);
...
Download(HWND hWnd, char *pcFileName)
{
    ...
    //
    if(Maverick_Open(g_ulDriveNum, g_pcFileName, 6) == 0)
    {
        GlobalFree(g_pvData);
        CloseHandle(g_hFile);
        return(FALSE);
...
Upload(HWND hWnd, char *pcFileName, char *pcDeviceName)
{
    ...
    //
    if(Maverick_Open(g_ulDriveNum, pcDeviceName, 1) == 0)
    {
        GlobalFree(g_pvData);
        return(FALSE);
    }
...
UpdateThread(void *pvParameter)
{
    ...
    //
    if(Maverick_Create(0, "", g_ulLength) == 0)
    {
        GlobalFree(g_pvData);
        CloseHandle(g_hFile);
        *pdwResult = FALSE;
...
Update(HWND hWnd, char *pcFileName)
{
    ...
    // Read data from the host file.
    //
    ReadFile(g_hFile, g_pvData, 1024 * 1024, &dwResult, NULL);
    ...
    //
    if(((((unsigned long *)g_pvData)[0] + 8) != g_ulLength) ||
...
{
    OPENFILENAME ofn;
    char buffer[4096], *pcFileNameStart;
    unsigned long ulLength;
    ...
    if(GetOpenFileName(&ofn) != 0)
    {
        ...
        //
        if(ofn.lpstrFile[strlen(buffer) + 1] == 0)
        {
            //
            // A single file was selected, so download it now.
            //
            DoDownload(hWnd, buffer);
        }
        else
        {
            //
            ...
            // the file name).
            //
            pcFileNameStart = buffer + strlen(buffer);
...
{
    OPENFILENAME ofn;
    char buffer[4096], *pcDeviceName, cAudibleData[512];
    ...
    if(GetOpenFileName(&ofn) != 0)
    {
                     ...
                     AudibleDlgProc) == IDOK)
        {
            ...
            //
            // Create the Audible meta-data for this file.
            //
            memset(cAudibleData, 0, 512);
            strncpy(pAudibleData->cTitle, g_pcAudibleTitle, 256);
            pAudibleData->ulProgramStart = g_ulAudibleStart;
            pAudibleData->ulSections = g_ulAudibleSections;
            pAudibleData->ulSectionStart[0] = g_pulAudibleSection[0];
            pAudibleData->ulSectionStart[1] = g_pulAudibleSection[1];
            pAudibleData->ulSectionStart[2] = g_pulAudibleSection[2];
            pAudibleData->ulSectionStart[3] = g_pulAudibleSection[3];
            pAudibleData->ulSectionStart[4] = g_pulAudibleSection[4];

            //
            // Add the Audible meta-data for this file to the device.
            //
            SetAudibleMetaData(pcDeviceName, cAudibleData, 508);

            //
            // A single file was selected, so download it now.
            //
            Download(hWnd, buffer);
        }
    }
    ...
    if(GetSaveFileName(&ofn) != 0)
    {
        ...
        while(iNumItems)
        {
            ...
            if(item.state & LVIS_SELECTED)
            {
                ...
                //
                strcpy(cPath + ofn.nFileOffset, cFile);

                //
                // Upload this file.
                //
                Upload(hWnd, cPath, cFile);
...
DeleteFiles(HWND hWnd)
{
    ...
    if(iNumItems == 1)
    {
        strcpy(msg, "Are you sure you want to delete this file?");
    }
    else
    {
        wsprintf(msg, "Are you sure you want to delete these %d files?",
                 iNumItems);
    }
    ...
    // these files.
    //
    if(MessageBox(hWnd, msg, "Confirm File Delete",
    ...
    while(iNumItems)
    {
        ...
        if(item.state & LVIS_SELECTED)
        {
            ...
            //
            Maverick_Delete(g_ulDriveNum, item.pszText);

            //
            // Remove any Audible meta-data associated with this file.
            //
            RemoveAudibleMetaData(item.pszText);
...
HandleCreate(HWND hWnd)
{
    ...
    //
    if(Maverick_GetDeviceName(pcBuffer, 256) == 1)
    {
        long lIdx;

        //
        // Convert the UNICODE string to ASCII.  This is making the assumption
        // that the UNICODE string is in US English.
        //
        for(lIdx = 2; lIdx < 256; lIdx += 2)
        {
            pcBuffer[lIdx / 2] = pcBuffer[lIdx];
        }
...
MainWndProc(HWND hWnd, UINT message, UINT wParam, LONG lParam)
{
    ...
    switch(message)
    {
        ...
        {
            //
            // Send this message to the lsit view control.
            //
