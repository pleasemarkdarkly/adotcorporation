...
//
//****************************************************************************
extern unsigned long g_ulDriveNum;
...
ReadMetaData(char **ppcBuffer, unsigned long *pulLength)
{
    //
    // Open the Audible meta-data file.
    //
    if(Maverick_Open(g_ulDriveNum, "\\audible.dat", 3) == 0)
    {
        //
...
//****************************************************************************
static void
WriteMetaData(char *pcBuffer, unsigned long ulLength)
{
    //
    // Open the Audible meta-data file.
    //
    if(Maverick_Open(g_ulDriveNum, "\\audible.dat", 3) == 0)
    {
        ...
        //
        if(Maverick_Open(g_ulDriveNum, "\\audible.dat", 7) == 0)
        {
            return;
        }
    }
...
//****************************************************************************
static unsigned long
FindFileName(char *pcBuffer, unsigned long ulLength, char *pcFileName)
{
    ...
    while(ulOffset)
    {
        ...
        for(ulIdx = 0; ulIdx < 4; ulIdx++)
        {
            ...
            //
            if(strncmp(pcBuffer + ulOffset + (ulIdx * 127) + 4, pcFileName,
                       123) == 0)
            {
                break;
            }
...
//****************************************************************************
unsigned long
AddFileName(char **ppcBuffer, unsigned long *pulLength, char *pcFileName)
{
    unsigned long ulOffset, ulIdx;
    ...
    //
    strncpy((*ppcBuffer) + ulOffset + (ulIdx * 127) + 4, pcFileName, 123);
...
//****************************************************************************
unsigned long
GetAudibleMetaData(char *pcFileName, char *pcData, unsigned long ulSize)
{
    char *pcBuffer;
    ...
    //
    if(ReadMetaData(&pcBuffer, &ulLength) == 0)
    {
        return(0);
    }
    ...
    //
    ulOffset = FindFileName(pcBuffer, ulLength, pcFileName);
    if(ulOffset == -1)
    {
        GlobalFree(pcBuffer);
        return(0);
...
//****************************************************************************
unsigned long
SetAudibleMetaData(char *pcFileName, char *pcData, unsigned long ulSize)
{
    char *pcBuffer;
    unsigned long ulCopy, ulOffset, ulLength;
    ...
    if(ulOffset == -1)
    {
        ...
        //
        ulOffset = AddFileName(&pcBuffer, &ulLength, pcFileName);
        if(ulOffset == -1)
        {
            GlobalFree(pcBuffer);
            return(0);
    ...
    //
    WriteMetaData(pcBuffer, ulLength);
...
//****************************************************************************
unsigned long
RemoveAudibleMetaData(char *pcFileName)
{
    char *pcBuffer;
