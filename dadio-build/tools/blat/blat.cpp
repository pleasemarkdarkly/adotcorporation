...
const NMLINES=(sizeof(usage) / sizeof(char *));
...
int prepare_smtp_message(char * MailAddress, char * dest,
...
int fixup(char * string, char * * tempstring2)
{
	...
	stl=strlen(string);
	for ( i=0;i<stl;i++ ) {
		if ( (string[i]<0x20) || (string[i]>0x7D) || (string[i] == 0x3D) ) needfix=1;
	}
	...
	if ( needfix ) {
		...
		strcpy(tempstring,"=?ISO-8859-1?Q?");
		for ( i=0;i<stl;i++ ) {
			 ...
			 char **envp )									 /* Array of environment variable strings    */
{
	...
	for ( next_arg=1;next_arg < argc;next_arg++ ) {
		...
		// is argv[2] "-profile"? If so, list or delete the profiles
			...
			if ( ! quiet )	for ( int i=0;i<NMLINES;i++ )	printMsg("%s\n",usage[i]); //lint !e578 (hiding i)
			return(1);
		}
	...
	if ( lstrcmp(filename, "-") == 0 ) {
		...
		// TF added check for temp file failure, copied older commented out code below.
		// create a unique temporary file name
		if ( NULL == GetTempPath( MAX_PATH, tempdir ) ) {
			 if ( ! quiet ) printMsg("error opening temporary file aborting\n");
			 delete [] Recipients;
			 return(13);
		}
      //GetTempFileName( tempdir, "blt", 0, tempfile );
		...
		tf = fopen(tempfile,"w");
		if ( tf==NULL ) {
			 if ( ! quiet ) printMsg("error opening temporary file %s, aborting\n",tempfile);
			 delete [] Recipients;
			 return(13);
		}*/
...
// Modification C.Henquin, 1998-06-12: Add support for wildcard
	...
	for ( k=1; k<=n_of_try || n_of_try == -1; k++ ) {
		...
		retcode = prepare_smtp_message( loginname, Recipients, my_hostname_wanted,AUTHLogin,AUTHPassword);
		if ( 0 == retcode ) {
