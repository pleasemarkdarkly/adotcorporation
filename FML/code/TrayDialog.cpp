...
void CTrayDialog::EnableMenu(bool bEnable)
{
	unsigned int menuItemID;
	int numItems = m_mnuTrayMenu.GetMenuItemCount();
	for (int i = 0; i < numItems; i ++)
	{
		menuItemID = m_mnuTrayMenu.GetMenuItemID(i);
		m_mnuTrayMenu.EnableMenuItem(menuItemID,bEnable);
	}
}

void CTrayDialog::OnSize(UINT nType, int cx, int cy) 
{
