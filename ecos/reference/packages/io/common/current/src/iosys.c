...
cyg_io_write(cyg_io_handle_t handle, const void *buf, cyg_uint32 *len)
{
    ...
    if (t->name[5] == 'u' && t->name[6] == 's' && t->name[7] == 'b') {
	/* added this hack to make the usb driver work without breaking other drivers.
	 * usb must be able to write 0 length packets. */
    }
    else if (NULL != len && 0 == *len) {
        return ENOERR;
    }
