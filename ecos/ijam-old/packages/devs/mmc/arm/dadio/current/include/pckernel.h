...
extern cyg_mutex_t mmc_fat_mutex;
...
/* 
    These macros are invoked at ap  once = 1;
proprite times by the source code. In a 
...
#define PC_FS_ENTER()    cyg_mutex_lock( &mmc_fat_mutex );
    /* This is invoked at the top of each API call. In 
			 ...
			 If finer grained multitasking is required the#include <cyg/kernel/kapi.h>
			 process managment is done with other macros
			 so this routine is not needed.
		       */                        
#define PC_FS_EXIT()     cyg_mutex_unlock( &mmc_fat_mutex );
    /* This macro is invoked each time rtfs exits
