...
OrinocoDSR(cyg_vector_t Vector, cyg_ucount32 Count, cyg_addrword_t Data) 
{
    ...
    //diag_printf("T:%x %d\n", (unsigned int)cyg_current_time(), __LINE__);
...
OrinocoDeliver(struct eth_drv_sc * SC)
{
    ...
    /* TODO This is a complete hack.  Problem is that interrupt nesting is getting messed up so that interrupts
       end up permanently disabled.  Why this only happens on Njord and not Dharma 2, I don't know */
...
PutParameters(IFBP pIFB)
{
    ...
    static unsigned char MulticastAddresses[] = { 0x01, 0x00, 0x5e, 0x7f, 0xff, 0xfa };
    ...
    /* Set up the multicast address for Njord */
    Status = hcf_put_info(pIFB, FillLTV(pLTVB, sizeof(MulticastAddresses), CFG_GROUP_ADDR, &MulticastAddresses[0]));
    if (Status != HCF_SUCCESS) {
	diag_printf("No space for multicast address\n");
    }
...
OrinocoCardHandler(cyg_addrword_t Param)
{
    ...
    while (true) {
		...
		if (CISBuffer[2] == LAN_NID) {
		    ...
		    *(volatile cyg_uint8 *)PBDR &= ~(PCMCIA_REG | PCMCIA_8_16_SELECT | PCMCIA_IO_SELECT);
...
OrinocoStop(struct eth_drv_sc * SC)
{
	...
	/* Disable the card */
	Status = hcf_action(pIFB, HCF_ACT_INT_OFF);
	    ...
	    int TotalLength, unsigned long Key) 
{
    ...
    /* Try this fix that marc @ agere suggested */
    
    /* Wait until RscInd free here and not in CanSend.
       Hope that this does not lose an Rx packet. */
    ...
    hcf_put_data(pIFB, TempBuf, j, HCF_PORT_0);
...
void 
in_port_string(unsigned int prt, char * dst, int n)
{
	//diag_printf("IP %d %x\n", n, dst);

	while ( n-- ) { 
		int i = IN_PORT_WORD( prt ); 
		*dst++ = (char)i; 
		*dst++ = (char)(i >> 8); 
	}
}
...
void
out_port_string(unsigned int prt, char * src, int n)	
{
	//diag_printf("OP %d %x\n", n, src);
	while ( n-- ) { 
		OUT_PORT_WORD( prt, *src | *(src+1)<<8  ); 
		src  += 2;
	}
}
