...
int noprintf(char *fmt, ...){ ++g_cShunted; return 0;}
int g_cShunted = 0;
int g_bshunting = 0;
static char g_printf_shuntstore[256];
...
{
    SHUNT_PRINT("ELF images not supported\n");
    return 0;
}
...
load_srec_image(int (*getc)(void), unsigned long base)
{
    ...
    while ((c = (*getc)()) > 0) {
        ...
        // Start of line
        if (c != 'S') {
            SHUNT_PRINT("Invalid S-record at offset %p, input: %c\n", (void *)offset, c);
            return 0;
        }
        type = (*getc)();
        offset += 2;
        ...
        if ((count = _hex2(getc, 1, &sum)) < 0) {
            SHUNT_PRINT("Bad S-record count at offset %p\n", (void *)offset);
            return 0;
        }
        offset += 1;
        switch (type) {
        ...
        case '3':
            ...
            if ((unsigned long)(addr-addr_offset) < lowest_address) {
				...
				SHUNT_PRINT("lowest addr in load srec is %x\n",lowest_address);
            }
            if ((addr < user_ram_start) || (addr > user_ram_end)) {
                SHUNT_PRINT("Attempt to load S-record data to address: %p [not in RAM]\n", (void*)addr);
                return 0;
            }
            count -= ((type-'1'+2)+1);
            offset += count;
            ...
            cksum = (~cksum & 0xFF);
            if (cksum != sum) {
                SHUNT_PRINT("*** Warning! Checksum failure - Addr: %lx, %02lX <> %02lX\n", (unsigned long)base_addr, sum, cksum);
                return 0;
            }
        ...
        case '9':
			...
			sprintf(g_printf_shuntstore,"Entry point: %p, address range: %p-%p\n", 
                   (void*)entry_address, (void *)lowest_address, (void *)highest_address);
            while ((c = (*getc)()) > 0) ;  // Swallow rest of data
            return highest_address;
        default:
            SHUNT_PRINT("Invalid S-record at offset 0x%lx, type: %x\n", (unsigned long)offset, type);
            return 0;
        }
        while ((c = (*getc)()) != '\n') offset++;
    }
...
redboot_getc(void)
{
    ...
    if (getc_info.avail == 0) {
        if (getc_info.verbose) {
            SHUNT_PRINT("%c\b", spin[getc_info.tick++]);
...
#define MODE_USB    (xyzModem_zmodem + 1)
...
do_load(int argc, char *argv[])
{
    ...
    init_opts(&opts[3], 'm', true, OPTION_ARG_TYPE_STR, 
              (void **)&mode_str, (bool *)&mode_str_set, "download mode (TFTP or xyzMODEM or USB)");
    num_options = 4;
#ifdef CYGPKG_REDBOOT_NETWORKING
    ...
    if (hostname_set) {
        if (!inet_aton(hostname, (in_addr_t *)&host)) {
            SHUNT_PRINT("Invalid IP address: %s\n", hostname);
            return;
        }
    }
#endif
    if (mode_str_set) {
        if (strncmpci(&mode_str[1], "modem", strlen(&mode_str[1])) == 0) {
            ...
            mode = MODE_TFTP;
            if (!have_net) {
                SHUNT_PRINT("TFTP mode requires a working network\n");
                return;
            }
#endif
#ifdef CYGPKG_REDBOOT_USB
	} else if (strcmpci(mode_str, "usb") == 0) {
	    mode = MODE_USB;
#endif
        } else {
            SHUNT_PRINT("Invalid 'mode': %s\n", mode_str);
    ...
    if (((mode == MODE_TFTP) || (mode == MODE_USB)) && !filename) {
        SHUNT_PRINT("File name missing\n");
        SHUNT_PRINT("usage: load %s\n", usage);
        return;
    }
    if (base_addr_set &&
        ((base < (unsigned long)user_ram_start) ||
         (base > (unsigned long)user_ram_end))) {
        SHUNT_PRINT("Specified address (%p) is not in RAM.\n", (void*)base);
        return;
    }
    if (raw && !base_addr_set) {
        SHUNT_PRINT("Raw load requires a memory address\n");
        return;
    }
#ifdef CYGPKG_REDBOOT_NETWORKING
    if (mode == MODE_TFTP) {
	...
	res = usb_stream_open(filename, &err);
	if (res < 0) {
	    SHUNT_PRINT("Can't load '%s': %s\n", filename, usb_error(err));
	    return;
	}
	redboot_getc_init(usb_stream_read, verbose);
    }
#endif
    else {
		SHUNT_PRINT("going quiet\n");
		g_bshunting = 1;
		SHUNT_PRINT("if this prints, then shunting is busted.\n");
    ...
    if (raw) {
        ...
        if (0 == err)
		{
            SHUNT_PRINT("Raw file loaded %p-%p\n", (void *)base, (void *)end);
		}
    } else {
        ...
        if (res >= 0) {
            ...
            if (strncmp(&type[1], "ELF", 3) == 0) {
                ...
                SHUNT_PRINT("Unrecognized image type: %lx\n", *(unsigned long *)type);
            }
        }
    }
#ifdef CYGPKG_REDBOOT_NETWORKING
...
#ifdef CYGPKG_REDBOOT_USB
    if (mode == MODE_USB) {
	usb_stream_close(&err);
    } else
#endif	
    if (mode != MODE_TFTP) {
        xyzModem_stream_close(&err);
		g_bshunting = 0;
		printf("%d prints shunted\n",g_cShunted);
		SHUNT_PRINT("%s",g_printf_shuntstore);
    }
    return;
}
