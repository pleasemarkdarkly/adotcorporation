...
void
venus_hal_hardware_init(void)
{
    // Venus boots in 16 bit mode
...
#define CS0_CONFIG MEMCFG_BUS_WIDTH_16 | MEMCFG_WAIT_STATES(0xF)
#define CS1_CONFIG 0x00
#define CS2_CONFIG 0x00
#define CS3_CONFIG MEMCFG_BUS_WIDTH_16 | MEMCFG_WAIT_STATES(0xF)
#define CS4_CONFIG MEMCFG_BUS_WIDTH_8 | MEMCFG_WAIT_STATES(0x6)
#define CS5_CONFIG MEMCFG_BUS_WIDTH_8 | MEMCFG_WAIT_STATES(0x0) | MEMCFG_CLKENB
#define CS6_CONFIG MEMCFG_BUS_WIDTH_32 | MEMCFG_WAIT_STATES(0)
#define CS7_CONFIG MEMCFG_BUS_WIDTH_8 | MEMCFG_WAIT_STATES(0)

    *(volatile cyg_uint32 *)MEMCFG1 =
        (CS0_CONFIG << 0) |       // FLASH rom
        (CS1_CONFIG << 8) |       // Unused
        (CS2_CONFIG << 16) |      // Unused
        (CS3_CONFIG << 24);       // CPLD
    ...
    *(volatile cyg_uint8 *)PADDR = 0x40;
	// Keyboard data 0-3 input
	// 4 - AC/DC input
	// 5 - nUSBCS input
	// 6 - DAC/ADC reset output
	// 7 - Stereo/Mono input from radio
    *(volatile cyg_uint8 *)PADR = 0x00;
    *(volatile cyg_uint8 *)PBDDR = 0x80;
	// 0 - Rotary encoder push input
	// 1 - TriPath AMP fault input
	// 2 - Line-in detect input or Mute output
	// 3 - Headphone-in detect input
	// 4 - Battery voltage check input
	// 5 - Rotary encoder input
	// 6 - Rotary encoder input
	// 7 - IDE reset, active low
    *(volatile cyg_uint8 *)PBDR = 0x00;
    *(volatile cyg_uint8 *)PDDDR = 0x3b;
	// 0 - DAC/ADC SCL output
	// 1 - DAC/ADC SDA output
	// 2 - Unused
	// 3 - Radio clock output
	// 4 - Radio data output
	// 5 - Radio write enable output
	// 6 - Unused
	// 7 - Unused
    *(volatile cyg_uint8 *)PDDR = 0x00;
    
    // Initialize system control
    *(volatile cyg_uint32 *)SYSCON2 = SYSCON2_KBWEN | SYSCON2_KBD6;
}

void
njord_hal_hardware_init(void)
{
#if (CYGHWR_HAL_ARM_EDB7XXX_PROCESSOR_CLOCK == 90000)
    *(volatile cyg_uint32 *)MEMCFG1 = 0x3c801810;
    *(volatile cyg_uint32 *)MEMCFG2 = 0x0000001b;
#else // (CYGHWR_HAL_ARM_EDB7XXX_PROCESSOR_CLOCK == 73728)
    *(volatile cyg_uint32 *)MEMCFG1 = 0x3c001854;
    *(volatile cyg_uint32 *)MEMCFG2 = 0x0000001a;
#endif

    // Set up GPIO lines
    *(volatile cyg_uint8 *)PADDR = 0xc0;
    // Keyboard data 0-5 input
    ...
    *(volatile cyg_uint8 *)PBDDR = 0xff;
    // 0 - PCMCIA reset
    // 1 - PCMCIA reg
    // 2 - PCMCIA 8/16 select
    // 3 - PCMCIA io select
    // 4 - PCMCIA voltage SLA0
    ...
    *(volatile cyg_uint8 *)PBDR = 0xe9;
    // PCMCIA reset and 8/16 asserted
    *(volatile cyg_uint8 *)PDDDR = 0x0a;
    // 0 - Memory module, Diagnostic LED control
    // 1 - PCMCIA card detect
...
#if defined(__VENUS)
    venus_hal_hardware_init();
#elif defined(__NJORD)
    njord_hal_hardware_init();
#else
#define MEMCFG_BUS_WIDTH(n)   (n<<0)
...
#define CS7_CONFIG MEMCFG_BUS_WIDTH_32 | MEMCFG_WAIT_STATES(0)
...
#if (CYGHWR_HAL_ARM_EDB7XXX_PROCESSOR_CLOCK == 90000)
    *(volatile cyg_uint32 *)MEMCFG1 = 0x3C801810; // 4(rnd)/2(seq) wait states does not work at 90MHz
#else
    *(volatile cyg_uint32 *)MEMCFG1 = 0x3C001814 | (0x40);  // turn on sqaen for ROM
#endif
    *(volatile cyg_uint32 *)MEMCFG2 = 0x0000001a;
    ...
    *(volatile cyg_uint8 *)PBDDR   = 0xFA;  // 0 - (PRDY1) PCMCIA
                                             ...
                                             // 6 - Memory module, On-board NAND Select (active low)
                                             // 7 - Memory module, SmartMedia Card Enable (active low)
    *(volatile cyg_uint8 *)PBDR    = 0xe0;  // Everything off
#endif
#if defined(__WINJAM)
...
#else
    *(volatile cyg_uint8 *)PDDDR   = 0x00;  // 0 - Memory module, Diagnostic LED control
                                             ...
                                             // 6 - (SDQM0)
                                             // 7 - (SDQM1)
    *(volatile cyg_uint8 *)PDDR    = 0x31;  // Everything off
#endif
#if defined(__WINJAM)
...
#if !defined(__DHARMA) && !defined(__VENUS)
