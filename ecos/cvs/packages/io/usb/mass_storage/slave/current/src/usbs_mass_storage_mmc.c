...
/* TODO Fix this */
#if 1
#include "/h/dadio/player/dar/atadrv/include/atadrv.h"
#else
...
#define IO_BLK_SET_CONFIG_MEDIA_STATUS_CB 0x0a85
#endif
...
static const char * const lun_name[MAX_LUNS + 1] = { "/dev/hda/", 
#if (MAX_LUNS > 0)
													 "/dev/hdb/"
#endif
 };
...
static const inquiry_data_t inquiry_data =
{
    ...
    vendor_identification: "Nikko   ",
    product_identification: "WMA Audio Player",
...
void
usbs_ms_protocol_init( usbs_mass_storage* ms )
{
    int status;
    int i;
	...
	status = cyg_io_lookup(lun_name[i], &(mmc_data->lun_handle[i]) );
	if (status < 0) {
	    DBG(("%s error lookup card %d\n", __FUNCTION__, status));
	}
#if defined(IO_PM_SET_CONFIG_REGISTER)
	{
	    cyg_uint32 len = 1;
	    status = cyg_io_set_config(mmc_data->lun_handle[i], IO_PM_SET_CONFIG_REGISTER, 0, &len);
	    if (status < 0) {
		DBG(("%s could not register drive %d\n", __FUNCTION__, status));
	    }
	}
#endif
...
#define NUM_SUPPORTED_PAGES 2
		...
		case UNIT_SERIAL_NUMBER_PAGE:
		{
		    ...
		    int dg_stat;
		    int len;
		    cyg_uint8 page[sizeof(unit_serial_number_t) + sizeof(dg.serial_num)];
		    ...
		    if (dg_stat == ENOERR) {
			memcpy(&sn->product_serial_number, &dg.serial_num, sizeof(dg.serial_num));
		    }
		    else {
			memset(&sn->product_serial_number, 0x20, sizeof(dg.serial_num));
		    }
   ...
   p 41 sbc */
{
	...
	*bytes_processed = 0;
	while (status >= 0 && sector < end_sector) {
	    /* Read block */
	    num_blks = (end_sector - sector) < (sizeof(mmc_data->data_buffer) / BLOCK_SIZE) ? (end_sector - sector) :
		(sizeof(mmc_data->data_buffer) / BLOCK_SIZE);
	...
	*bytes_processed = 0;
	while (status >= 0 && sector < end_sector && !mmc_data->reset_receive) {
		...
		*bytes_processed += ms->rx_result;
		...
		num_blks = ms->rx_result / BLOCK_SIZE;
