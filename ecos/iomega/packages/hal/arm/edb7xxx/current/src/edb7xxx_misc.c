...
#define CS7_CONFIG MEMCFG_BUS_WIDTH_32 | MEMCFG_WAIT_STATES(0)
    ...
    *(volatile cyg_uint32 *)MEMCFG1 = 0x02000014;
    // BrianW wanted the waitstates to 8 - (4 move x91) is (x81 for 8)
    *(volatile cyg_uint32 *)MEMCFG2 = 0x00008100;
    // Set up GPIO lines
    ...
    *(volatile cyg_uint8 *)PBDDR   = 0x5f; /* 0 - clik reset, active low
					    ...
					    * 6 - headphone mute, mute(low) on(high)
					    * 7 - VBUS_STATUS vbus present(high */
    *(volatile cyg_uint8 *)PBDR    = 0x12; /* clik in reset mode, active
					    * ATA_EN inactive, mystic off
					    * power on
					    * headphone muted */
    *(volatile cyg_uint8 *)LEDFLSH = 0x00; /* turn off LEDFLSH since it affects port D */
    *(volatile cyg_uint8 *)PDDDR   = 0xf0; /* 0 -
					    * 1 - backlight lcd, active high
					    * 2 - CHARGE_DISABLE, active high
					    *     Clik/Mystic power in phase 3, active low
    ...
    *(volatile cyg_uint8 *)PDDR    = 0x04; /* backlight off
					    ...
					    * lcd in reset mode */
#endif
    *(volatile cyg_uint8 *)PEDDR   = 0x07; /* 0 - EXTCLK control on(low) off(high), used for dac
