...
usb_thread(cyg_addrword_t data)
{
	...
	if (current_time < timeout) {
	    ...
	    /* if mystic is not already enabled */
	    if (*(volatile cyg_uint8 *)PBDR & 0x02) {
	    ...
	    *(volatile cyg_uint8 *)PBDR &= ~0x02;
	    ...
	    /* record initial disk activity */
	    disk_activity = (*(volatile cyg_uint8 *)PBDR & 0x40) >> 6;
	    if (callback) {
		callback(USB_DISK_ACTIVITY + disk_activity);
	    }
		...
		/* update disk activity if it changes */
		if (((*(volatile cyg_uint8 *)PBDR & 0x40) >> 6) != disk_activity) {
		    disk_activity = 1 - disk_activity;
		    if (callback) {
			callback(USB_DISK_ACTIVITY + disk_activity);
		    }
		}
