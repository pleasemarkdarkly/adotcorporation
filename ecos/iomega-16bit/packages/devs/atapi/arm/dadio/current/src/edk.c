...
BYTE PollStatusWithTimeout(BYTE clear_bits, BYTE set_bits, TIMER timeout);
...
BYTE ResetAtapi(void)
{
    ...
    if (PollStatusWithTimeout(STATUS_BSY, 0, 100))/* 1 second timeout */
    {
#if BIT_RESET
	for (reset_count = 1; reset_count < 3; ++reset_count)
	{
	    WriteControl(CONTROL_SRST);
	    for (i = 0; i < 1000; ++i);	/* delay of at least 5 usec */
	    WriteControl(CONTROL_NORMAL);

	    if (!PollStatusWithTimeout(STATUS_BSY, 0, 100))
	    {
		sleep_flag = 0;
		return (NO_ERROR);
	    }
...
BYTE InitAtapi(void)
{
	...
	// (epg,8/15/2000): TUR needed after media changed before drive access will succeed.
	// I think this would fit the bill, but we've already added in a full re-init elsewhere.
	// The following doesn't seem to hurt anything, but is unnecessary with the re-init in place.
	// Since we seem to be happy with the re-init, I'll let this drop.
//	if (ret_val == ERROR_MEDIA_CHANGED)
//		DriveStatus();
...
#define COUNT_DRIVE_POLLS 0
BYTE PollStatus(BYTE clear_bits, BYTE set_bits)
{
    TIMER timeout;
    TIMER ticks;
#if COUNT_DRIVE_POLLS
    int nPolls = 0;
#endif
...
BYTE PollStatusWithTimeout(BYTE clear_bits, BYTE set_bits, TIMER timeout)
{
    ...
    //diag_printf("PollStatusWithTimeout enter\n");
	...
	if (ticks >= timeout) {
	    diag_printf("PollStatusWithTimeout exit ERROR_TIMEOUT\n");
	    return(ERROR_TIMEOUT);
	}
    }
    //diag_printf("PollStatusWithTimeout exit\n");
    return(NO_ERROR);	
}
