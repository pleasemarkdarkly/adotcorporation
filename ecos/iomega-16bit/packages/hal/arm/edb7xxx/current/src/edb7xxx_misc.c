...
#define CS7_CONFIG MEMCFG_BUS_WIDTH_32 | MEMCFG_WAIT_STATES(0)
    ...
    *(volatile cyg_uint32 *)MEMCFG2 = 
    ...
    /* correct MEMCFG values */
    *(volatile cyg_uint32 *)MEMCFG1 = 0x03000014;
    // BrianW wanted the waitstates to 8 - (4 move x91) is (x81 for 8)
    *(volatile cyg_uint32 *)MEMCFG2 = 0x00008000;
    // Set up GPIO lines
    *(volatile cyg_uint8 *)PADDR   = 0x00; // Keyboard data 0-7 input
#ifdef IGNORE_LOCK_HACK
    *(volatile cyg_uint8 *)PADDR   = 0x10;
	*(volatile cyg_uint8 *)PADR    = 0x00;
#endif
    *(volatile cyg_uint8 *)PBDR    = 0x10; /* make sure power pin is high before setting as output
					    * to prevent ourselves from turning off */
    *(volatile cyg_uint8 *)PBDDR   = 0x0f; /* 0 - clik reset, active low
					    * 1 - ATA_EN enable mystic, active low
					    * 2 - SCL clock dac
					    * 3 - SDA data dac
					    * 4 - Player power, active high
					    * 5 - PWR500 usb power status from mystic,
					    *     Wall power supply detection in phase 3, active high
					    * 6 - CLIK_STATUS
					    * 7 - VBUS_STATUS vbus present(high) */

    /* if USB plugged in, don't disable mystic */
    if (*(volatile cyg_uint8 *)PBDR & 0x80) {
	*(volatile cyg_uint8 *)PBDR = 0x00; /* ATA_EN enabled */
    }
    else {
	*(volatile cyg_uint8 *)PBDR    = 0x02; /* clik in reset mode, active
						* ATA_EN inactive, mystic off */
    }
    
    *(volatile cyg_uint8 *)LEDFLSH = 0x00; /* turn off LEDFLSH since it affects port D */
    *(volatile cyg_uint8 *)PDDDR   = 0xf1; /* 0 - HW rev. level, LSB
					    * 1 - backlight lcd, active high
					    * 2 - Clik/Mystic power, active low
					    * 3 - LCD_RESET, reset(low), on(high)
					    * 4 - CHARGE_DET
					    * 5 - BAT_LEV1
					    * 6 - BAT_LEV2
					    * 7 - HW rev. level, MSB */
...
#ifdef PHASE_3
	if (*(volatile cyg_uint8 *)PBDR & 0x80) {
		*(volatile cyg_uint8 *)PDDR    = 0x00; /* backlight off
					    * lcd in reset mode */
	}
	else
	{
		*(volatile cyg_uint8 *)PDDR    = 0x04; /* backlight off
					    * lcd in reset mode */
	}
#else
    *(volatile cyg_uint8 *)PDDR    = 0x04; /* backlight off
					    ...
					    * lcd in reset mode */
#endif
    *(volatile cyg_uint8 *)PEDDR   = 0x03; /* 0 - EXTCLK control on(low) off(high), used for dac
					    * 1 - DAC reset, active low */
    *(volatile cyg_uint8 *)PEDR    = 0x00; /* EXTCLK on
					    * dac in reset mode */
