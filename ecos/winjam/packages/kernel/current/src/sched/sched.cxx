...
#include <cyg/hal/hal_intr.h>          // hal hardware clock read
#include <pkgconf/hal_arm_edb7xxx.h>   // fuck me.
#endif
...
#ifdef CYGFUN_KERNEL_THREADS_CPU_USAGE
				// Tinker with the cpu usage times just before the context switch
				// technically we'd like to read the RTC and the hardware clock at the
				// same time. but in truth we cant, so read the hardware clock first
				cyg_uint32 clock_ticks;
				HAL_CLOCK_READ( &clock_ticks );
				next->cpu_start_time = (Cyg_Clock::real_time_clock->current_value()*CYGNUM_HAL_RTC_PERIOD)
										+clock_ticks;
				current->cpu_total_time += (next->cpu_start_time - current->cpu_start_time);
#endif
