...
#ifdef CYGHWR_NET_DRIVER_ETH0_AUTO_IP
char autoIPaddress_eth0[20];
#endif

#ifdef CYGHWR_NET_DRIVER_ETH1_AUTO_IP
char autoIPaddress_eth1[20];
#endif
...
init_eth0(void) 
{
    ...
    if ( ! eth0_up ) { // Make this call idempotent
		...
		diag_printf("Trying DHCP on eth0...\n");
...
#endif
        {
		    ...
		    if (eth0_up) {
				...
				{
					diag_printf("Network initialized for eth0 using DHCP.\n");
				}
			}
			} else {
...
#ifdef CYGHWR_NET_DRIVER_ETH0_AUTO_IP
				diag_printf("Using Auto-IP on eth0\n");

				srand((unsigned int)cyg_current_time());

				sprintf(autoIPaddress_eth0,"169.254.%d.%d",rand()%254,rand()%254);
				diag_printf("Using address: %s\n",autoIPaddress_eth0);

				// shut down interface so we can re-initialize it
				do_dhcp_down_net(eth0_name,NULL,&eth0_dhcpstate,NULL);
			
				eth0_up = true;

				// clear the bootp record
				memset(&eth0_bootp_data,0,sizeof(struct bootp));

				// build a new bootp record
				build_bootp_record(&eth0_bootp_data,autoIPaddress_eth0,
							"255.255.0.0",		// subnet mask
							"169.254.255.255",	// IP broadcast
							"169.254.0.1",		// gateway
							"0.0.0.0");			// server
				...
				if (!init_net(eth0_name, &eth0_bootp_data)) {
					...
					diag_printf("Network initialized for eth0 with Auto-IP address.\n");
				}
        ...
        show_bootp(eth0_name, &eth0_bootp_data);
		 if (eth0_up) {
			...
			{	
				diag_printf("Network initialized (eth0).\n");
			}
...
init_eth1(void) 
{
    ...
    if ( ! eth1_up ) { // Make this call idempotent
		...
		diag_printf("Trying DHCP on eth1...\n");
...
#endif
        {
			...
			if (eth1_up) {
				...
				{
					diag_printf("Network initialized for eth1 using DHCP.\n");
				}
...
#ifdef CYGHWR_NET_DRIVER_ETH1_AUTO_IP
				diag_printf("Using Auto-IP on eth1\n");

				srand((unsigned int)cyg_current_time());
				sprintf(autoIPaddress_eth1,"169.254.%d.%d",rand()%254,rand()%254);

				// shut down interface so we can re-initialize it
				do_dhcp_down_net(eth1_name,NULL,&eth1_dhcpstate,NULL);
			
				eth1_up = true;
				memset(&eth1_bootp_data,0,sizeof(struct bootp));

				build_bootp_record(&eth1_bootp_data,autoIPaddress_eth1,
						"255.255.0.0",		// subnet mask
						"169.254.255.255",	// ip broadcast
						"169.254.0.1",		// gateway
						"0.0.0.0");			// server
				...
				if (!init_net(eth1_name, &eth1_bootp_data)) {
					...
					diag_printf("Network initialized for eth1 with Auto-IP address.\n");
				}
		...
		{
			diag_printf("Network initialized (eth1).\n");
		}
...
init_all_network_interfaces(void)
{
    ...
    // Initializing eth0 and eth1 used to be here
#ifndef __NJORD
	init_eth0();
	init_eth1();
#endif
