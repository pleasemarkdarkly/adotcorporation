...
net_init(void)
{
    ...
    config_get("net_debug", &net_debug, CONFIG_BOOL);
    config_get("gdb_port", &gdb_port, CONFIG_INT);
    config_get("bootp", &use_bootp, CONFIG_BOOL);
    config_get("bootp_my_ip", &__local_ip_addr, CONFIG_IP);
    config_get("bootp_server_ip", &my_bootp_info.bp_siaddr, CONFIG_IP);
#endif
    have_net = false;
