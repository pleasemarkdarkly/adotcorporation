...
{
	/// <summary>
	/// Summary description for ViewAttractLoopItems.
	/// </summary>
	public class ViewAttractLoopItems : System.Web.UI.Page
	{
		CSWContext _ctx;
		...
		public string EditCallback(DataRow row, ColumnBase df, int iRec)
		{
			if(Convert.ToInt32(row["cAssociations"]) == 0)
				return "/CentralWeb/AttractLoops/EditAttractLoopItem.aspx?idFile=" + row["idFile"];
			else
				return "";
		}
		...
		{
			if(Convert.ToInt32(row["cAssociations"]) == 0)
				return "/CentralWeb/AttractLoops/ViewAttractLoopItems.aspx?cmd=FileRemove&idFile=" + row["idFile"];
			else
				return "";
		}
		...
		public void DoListView()
		{
			...
			ListView lv = new ListView(_ctx, "ViewAttractLoopItems","registered_files", false, false);
