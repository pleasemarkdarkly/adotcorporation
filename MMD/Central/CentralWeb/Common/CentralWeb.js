	...
	for (var i = 0; i < frm.length; i++) 
		...
		if (eleValid.name.substring(0,8) == "required" && eleValid.value != "0")  // 0 is not req
			...
			case "nonnegLong":
				...
				break;
			case "positiveLong":
				if (!IsInteger(ele.value) || parseInt(ele.value) < 1)
				{
					return WarnValue(ele, eleLabel.value, "a positive integer.", true);
				}
				break;
			case "bool":
...
function FormAlert(sFrm, sTitle, sMsg, sDiv, dx, dy)
{
	...
	sUrl = "/CentralWeb/Common/Dialog/Form.aspx?" + Math.random();
...
function NiceAlertEx(iType, sTitle, sMsg, sOk, sCancel)
{
	...
	switch (parseInt(iType))
		{
	case 0:
		sUrl = "/CentralWeb/Common/Dialog/OK.aspx?" + Math.random();
   	break;
	case 1:
		sUrl = "/CentralWeb/Common/Dialog/YesNo.aspx?" + Math.random();
		arArgs[2] = sOk;
		arArgs[3] = sCancel;
		break;
	case 2:
		...
		break;
   	case 3:
		sUrl = "/CentralWeb/Common/Dialog/YesNoCookie.aspx?" + Math.random();
...
function ToggleBVT()
{
	...
	for(i=0;i<rules.length;i++)
		if(rules[i].selectorText == ".BVT")
			rules[i].style.display = (rules[i].style.display == "none") ? "block" : "none";
}