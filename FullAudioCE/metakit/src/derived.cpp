...
//
//@todo     implement propagation of sort, solve a few complex problems
//@todo     simplify this stuff, it's becoming far too large...
//@todo     _rowMap + _revmap use longs, should be shorts on 16-bit machines
...
/////////////////////////////////////////////////////////////////////////////
//@class:(IMPL) A filtering sequence is a derived sequence for row selection.
//
//  An array of index values is built and saved during construction. This
//  array determines which entries of the underlying sequence are accepted.
...
c4_FilterSeq::c4_FilterSeq (c4_Sequence& seq_)
    : c4_DerivedSeq (seq_)
{
    _rowMap.SetSize(_seq.Size());
    _revMap.SetSize(_seq.Size());
    d4_assert(Size() == _seq.Size());
    ...
    : c4_DerivedSeq (seq_), _lowRow (*low_), _highRow (*high_)
{
    ...
    _rowMap.SetSize(_seq.Size());   // avoid growing, use safe upper bound
...
c4_Notifier* c4_FilterSeq::PreChange(c4_Notifier& nf_)
{
    ...
    switch (nf_._type)
    {
        ...
        case c4_Notifier::kMove:
            {
                int i = PosInMap(nf_._index);
                bool inMap = i < Size() && (int) _rowMap.GetAt(i) == nf_._index;
        ...
        case c4_Notifier::kInsertAt:
            {
                int i = nf_._index < _revMap.GetSize() ?
                    (int) _revMap.GetAt(nf_._index) : Size();
...
/////////////////////////////////////////////////////////////////////////////
//:(IMPL) A sorting sequence is a derived sequence which sorts rows.
...
bool c4_SortSeq::LessThan(t4_u32 a, t4_u32 b)
{
    if (a == b)
        return false;
    ...
    : c4_FilterSeq (seq_), _info (0), _width (-1)
{
    ...
    if (Size() > 0)
    {
        ...
        MergeSort(& _rowMap.ElementAt(0), Size());
...
int c4_SortSeq::PosInMap(c4_Cursor cursor_) const
{
    ...
    d4_assert(i == Size() || Compare(i, cursor_) >= 0);
    return i;
}
        ...
        case c4_Notifier::kSetAt:
            {
                ...
                if ((oi > 0 && Compare(oi - 1, cursor) > 0) ||
                    (oi+1 < Size() && Compare(oi+1, cursor) < 0))
                {
                    _rowMap.RemoveAt(oi);
...
/////////////////////////////////////////////////////////////////////////////
//:(IMPL) A projecting sequence rearranges fields.
