...
//
//  This file contains the implementation of the customizing classes.
...
//  class c4_Handler;
        class c4_CustomHandler;         // a handler for custom sequences

//  class c4_Sequence;
//      class c4_HandlerSeq;
            class c4_CustomSeq;         // a sequence for customizable views

    class c4_CustomViewer;              // base class for custom views
        class c4_SliceViewer;
        class c4_ProductViewer;
        class c4_RemapWithViewer;
        class c4_PairViewer;
        class c4_ConcatViewer;
        class c4_RenameViewer;
        class c4_GroupByViewer;
        class c4_JoinPropViewer;
        class c4_JoinViewer;
...
c4_CustomHandler::c4_CustomHandler (const c4_Property& prop_,
                                            c4_CustomSeq* seq_)
    : c4_Handler (prop_), _seq (seq_), _col (-1)
{
    d4_assert(_seq != 0);
}
...
void c4_CustomHandler::Initialize(int col_)
{
    d4_assertThis;

    c4_Handler::Initialize(col_);

    _col = col_;
}

const void* c4_CustomHandler::Clear(int& length_) const
{
    d4_assertThis;

	static char zeros[8];
	
	length_ = f4_ClearFormat(Property().Type());
	d4_assert(length_ <= sizeof zeros);
    return zeros;
}
...
int c4_CustomHandler::Compare(int index_, const c4_Bytes& buf_)
{
    d4_assertThis;
...
void c4_CustomViewer::SetItem(int row_, int col_, const c4_Bytes& buf_)
{
	d4_assert(false); // default is not modifiable
}

void c4_CustomViewer::InsertRows(int pos_, c4_Cursor value_, int count_)
{
	d4_assert(false); // default is not modifiable
}

void c4_CustomViewer::RemoveRows(int pos_, int count_)
{
	d4_assert(false); // default is not modifiable
}
...
public: 
    ...
    virtual int GetSize();
    virtual bool GetItem(int row_, int col_, c4_Bytes& buf_);
	void SetItem(int row_, int col_, const c4_Bytes& buf_);
};
...
void c4_RemapWithViewer::SetItem(int row_, int col_, const c4_Bytes& buf_)
{
    d4_assertThis;
...
void c4_PairViewer::SetItem(int row_, int col_, const c4_Bytes& buf_)
{
    d4_assertThis;
...
void c4_ConcatViewer::SetItem(int row_, int col_, const c4_Bytes& buf_)
{
    d4_assertThis;
...
void c4_RenameViewer::SetItem(int row_, int col_, const c4_Bytes& buf_)
{
    d4_assertThis;
      ...
      _sub (sub_), _subPos (_parent.FindProperty(sub_.GetId())), _subWidth (0)
{
    ...
    for (int i = 0; i < _parent.GetSize(); ++i)
    {
        ...
        {
            _base.Add(i);
            _offset.Add(~ (t4_u32) 0);  // special null entry for outer joins
        }
        else
            for (int j = 0; j < n; ++j)
            {
    ...
    for (int i = 0; i < sorted.GetSize(); ++i)
    {
        ...
        else    // no, this is a new combination
        {
            ...
            else if (outer_)
            {
                ...
                _base.Add(orig);
                _offset.Add(~ (t4_u32) 0);  // special null entry
                ++n;
            }
        }
    }
